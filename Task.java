...
public interface Task {
   ...
   *         information that can be interpreted by the specific {@link Task} implementation.
   */
  TaskResult run();

  /**
   * Signals the task to stop execution. The task implementation should carry out any clean up
   * actions that may be required, including possible interruption and return from the {@link #run()} method.
   */
  void cancel();
}
